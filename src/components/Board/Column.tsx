...
const Column = ({ item, x, y, won, lost }: IColumn) => {
    ...
    if (ref.current != null) {
      ...
      if (isPenciel) {
        if ([1, 2, 3, 4, 5, 6, 7, 8, 9].includes(v)) {
          const tmpPencielBoard = [...pencielBoard];

          tmpPencielBoard[v - 1] =
            tmpPencielBoard[v - 1] === "" ? v.toString() : "";
  ...
  return (
    <>
      <li
        onClick={showAllSameNumberValue}
        className={`${styles.li} ${
          activeNumber == item && item != "" ? styles.active : ""
        }`}
      >
        <input
          ref={ref}
        ...
        <ul className={styles.pencialRow}>
          {pencielBoard.map((peciel, pecielKey) => (
            <li
              className={styles.pencialLi}
              key={`penciel-${item}-${pecielKey}`}
            >
              {peciel}
            </li>
          ))}
